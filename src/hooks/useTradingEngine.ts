...
export function useTradingEngine() {
          ...
          if ((bidPrices.length || askPrices.length) &&
            ...
            if (!initialPriceSet) {
              ...
              if (bestBid0 && bestAsk0) {
                ...
                console.log(`📍 CENTRAGE DOM: Prix ORDERBOOK initial=${p0}, bestBid=${bestBid0}, bestAsk=${bestAsk0}`);
                initialPriceSet = true;
                samplePricesRef.current.push(p0);
                // Force la reconstruction du ladder avec l'ancre
                setForceUpdate(prev => prev + 1);
  ...
  useEffect(() => {
    ...
    const newPnl = {
      unrealized: unreal,
      realized: sessionRealizedPnLRef.current,
      total: sessionRealizedPnLRef.current + unreal
    };
