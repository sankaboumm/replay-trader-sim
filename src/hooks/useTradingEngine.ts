...
// ––––– AGRÉGATION T&S (déclaré AVANT tout usage) –––––
...
// ––––– FILLS (déclaré AVANT tout usage) –––––
...
setPosition(prevPos => {
  ...
  } // sinon réduction sans flip → avg inchangé
...
// ––––– EVENT PROCESSOR (déclaré avant toute utilisation) –––––
...
const playNextEvent = () => {
  ...
  // Programmer le prochain événement s'il y en a un
  if (currentEventIndex + 1 < marketData.length) {
    const dt = Math.max(1, Math.floor(10 / playbackSpeed));
    playbackTimerRef.current = setTimeout(playNextEvent, dt);
  } else {
    // Fin de la lecture
    flushAggregationBuffer();
    setIsPlaying(false);
...
// Démarrer immédiatement le premier événement
playNextEvent();
...
// ––––– DÉRIVÉS BEST BID/ASK + SPREAD (affichage) –––––
